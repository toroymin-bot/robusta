#!/usr/bin/env node
/**
 * sim-live-plus-264h.mjs
 *   - C-D76-4 (D+3 15시 §8 슬롯, 2026-05-11) — Tori spec C-D76-4 (Task_2026-05-11 §5.6).
 *
 * Why: C-D76-1 check-live-plus-264h.mjs dry-run 시뮬레이터. C-D75-4 +24h 1:1 미러.
...
import { checkLivePlus264h } from "./check-live-plus-264h.mjs";

const ONE_HOUR_MS = 60 * 60 * 1000;
const EPOCH_T0_MS = Date.parse("2026-05-07T00:00:00+09:00");
const GATE_AT_MS = EPOCH_T0_MS + 264 * ONE_HOUR_MS;
const DEFAULT_SCENARIOS = ["gate-pre", "gate-pass", "gate-post-24h"];

function scenarioNow(name) {
  switch (name) {
    case "gate-pre":
      return GATE_AT_MS - ONE_HOUR_MS; // T+263h
    case "gate-pass":
      return GATE_AT_MS; // T+264h 정각
    case "gate-post-24h":
      return GATE_AT_MS + 24 * ONE_HOUR_MS; // T+288h
    default:
...
export async function simLivePlus264h(opts) {
  const o = opts || {};
  ...
  for (const name of scenarios) {
    ...
    const r = await checkLivePlus264h({ now, epochT0 });
...
async function main() {
  const result = await simLivePlus264h();
  ...
  console.log("");
  console.log(
    `sim:live-plus-264h — ${result.pass}/${result.total} ${result.fail === 0 ? "PASS" : "FAIL"} (${result.totalMs} ms)`,
  );
