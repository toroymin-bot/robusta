#!/usr/bin/env node
/**
 * check-live-plus-168h.mjs
 *   - C-D72-1 (D+2 19시 §8 슬롯, 2026-05-10) — Tori spec C-D72-1 (§7.5).
 *
 * Why: LIVE +168h(7일) ±windowMin 윈도우 read-only 게이트 (D-D71 C-D71-1 +24h 패턴 1:1 미러).
 *   LIVE 시작 후 168h 시점 ±windowMin (기본 60) 정합 검증 + pingsPath 카운트.
 *   외부 fetch 0건 / Dexie 미접근.
 *
 * 자율 정정 큐 (D-72-자-1):
 *   환경 변수 LIVE_PLUS_168H_WINDOW_MIN 으로 windowMin override 가능 (디폴트 60 보존, backward-compatible).
 *
 * 함수 시그니처 (named export, §7.5 본체 lock 정합):
 *   checkLivePlus168h({ liveStartIso, nowIso, windowMin, pingsPath })
 ...
 *     - windowMin:    int, default process.env.LIVE_PLUS_168H_WINDOW_MIN || 60.
 ...
 *   inWindow     boolean — 167 ≤ tPlusH ≤ 169 (windowMin=60 기준)
 ...
 *   3) tPlusH < 167 또는 tPlusH > 169 (windowMin=60) → ok=false, reasons=['out of window']
 *
 * 사용 (CLI):
 *   $ node scripts/check-live-plus-168h.mjs --live-start=2026-05-08T00:00:00+09:00
 *   $ LIVE_PLUS_168H_WINDOW_MIN=30 node scripts/check-live-plus-168h.mjs
 *
...
function envWindowMin() {
  const v = process.env.LIVE_PLUS_168H_WINDOW_MIN;
...
export function checkLivePlus168h(opts) {
  const o = opts || {};
  ...
  const lower = T_PLUS_168H_HOURS - windowH;
  const upper = T_PLUS_168H_HOURS + windowH;
...
function main() {
  ...
  try {
    const result = checkLivePlus168h({
    ...
    console.log(JSON.stringify({ livePlus168h: result }));
    console.log(
      `check:live-plus-168h ok=${result.ok} inWindow=${result.inWindow} tPlusH=${result.tPlusH.toFixed(2)} pingsTotal=${result.pingsTotal} reasons=${JSON.stringify(result.reasons)}`,
    );
    process.exit(result.ok ? 0 : 1);
  } catch (e) {
    console.error(`check:live-plus-168h ERROR: ${e.message}`);
    process.exit(1);
  }
}
