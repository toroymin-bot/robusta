#!/usr/bin/env node
/**
 * check-live-plus-120h.mjs
 *   - C-D70-1 (D+2 11시 §6 슬롯, 2026-05-10) — Tori spec C-D70-1 (§1.6).
 *
 * Why: LIVE +120h ±windowMin 윈도우 read-only 게이트 (D-D69 C-D69-1 패턴 미러).
 *   LIVE 시작 후 120h 시점 ±windowMin (기본 60) 정합 검증 + pingsPath 카운트.
 *   외부 fetch 0건 / Dexie 미접근.
 *
 * 자율 정정 큐 (D-70-자-1):
 *   환경 변수 LIVE_PLUS_120H_WINDOW_MIN 으로 windowMin override 가능 (디폴트 60 보존, backward-compatible).
 *
 * 함수 시그니처 (named export, §1.6 본체 lock 정합):
 *   checkLivePlus120h({ liveStartIso, nowIso, windowMin, pingsPath })
 ...
 *     - windowMin:    int, default process.env.LIVE_PLUS_120H_WINDOW_MIN || 60.
 ...
 *   inWindow     boolean — 119 ≤ tPlusH ≤ 121 (windowMin=60 기준)
 ...
 *   3) tPlusH < 119 또는 tPlusH > 121 (windowMin=60) → ok=false, reasons=['out of window']
 *
 * 사용 (CLI):
 *   $ node scripts/check-live-plus-120h.mjs --live-start=2026-05-08T00:00:00+09:00
 *   $ LIVE_PLUS_120H_WINDOW_MIN=30 node scripts/check-live-plus-120h.mjs
 *
...
function envWindowMin() {
  const v = process.env.LIVE_PLUS_120H_WINDOW_MIN;
...
export function checkLivePlus120h(opts) {
  const o = opts || {};
  ...
  const lower = T_PLUS_120H_HOURS - windowH;
  const upper = T_PLUS_120H_HOURS + windowH;
...
function main() {
  ...
  try {
    const result = checkLivePlus120h({
    ...
    console.log(JSON.stringify({ livePlus120h: result }));
    console.log(
      `check:live-plus-120h ok=${result.ok} inWindow=${result.inWindow} tPlusH=${result.tPlusH.toFixed(2)} pingsTotal=${result.pingsTotal} reasons=${JSON.stringify(result.reasons)}`,
    );
    process.exit(result.ok ? 0 : 1);
  } catch (e) {
    console.error(`check:live-plus-120h ERROR: ${e.message}`);
    process.exit(1);
  }
}
