...
// ── 게이트 8: emergency bypass commit 0건 (L-D58-3)
//   자율 정정 D-59-자-1 + D-59-자-3 (정합 lock):
//     (1) --grep 은 commit message body 까지 검색하므로, body 자기 인용한 다른 사이클
//         commit 이 false positive. subject (%s) 만 추출 + JS RegExp 매칭.
//     (2) 본 사이클 commit message subject 가 매우 긴 단일 라인이라 self-quote 가
//         subject 안에 직접 들어감. 단순 단어 매칭 + 등호 매칭으로도 자기 인용 매치.
//     (3) 검사 패턴을 정형 prefix `^\[BYPASS\]\s` (subject 시작 [BYPASS] ) 로 변경 —
//         emergency bypass 사용 시 SoP lock 정형 prefix 의무. 자기 인용 평문은 prefix
//         없음 → 미매치. verify-d59 G4 동일 패치.
{
  const since = "2026-05-07T23:00:00+09:00";
  const r = runGit([
    "log",
  ...
  if (hits.length === 0) {
    pass("게이트 8: emergency bypass commit 0건 (L-D58-3 정합 / [BYPASS] prefix 매칭)");
  } else {
    fail(
      "게이트 8",
