#!/usr/bin/env node
/**
 * check-live-plus-30h.mjs
 *   - C-D65-1 (D+1 07시 §4 슬롯, 2026-05-09) — Tori spec C-D65-1.
 *
 * Why: LIVE +30h ±15분 윈도우 정형 lock.
 *   submit + 30h 시점 ±15분 정합 검증 read-only 게이트.
 *   외부 fetch 0건 / Dexie 미접근 / 본 도구는 시점 산식만 책임.
 *
 * 자율 정정 (D-65-자-1):
 *   환경 변수 LIVE_PLUS_30H_WINDOW_MIN 으로 windowMin override 가능 (디폴트 15 보존, backward-compatible).
 *
 * 함수 시그니처 (default export):
 *   checkLivePlus30h({ submitKst, nowKst, windowMin?, freeze? })
 *     -> { ok, deltaMin, reason? }
 *
 *     - submitKst:  ISO 문자열 with +09:00.
 *     - nowKst:     ISO 문자열 with +09:00.
 *     - windowMin:  정수, 디폴트 15 (env LIVE_PLUS_30H_WINDOW_MIN override 가능).
 ...
 *   4) nowKst < submitKst+30h → deltaMin 음수, ok 가능
 *   5) freeze=true → 항상 ok=false, reason='freeze'
 *
 * 사용 (CLI):
 *   $ node scripts/check-live-plus-30h.mjs --submit=2026-05-08T07:00:00+09:00 \
 *       --now=2026-05-09T13:00:00+09:00
 *
 * 외부 dev-deps +0 (node 표준만).
 */

const ONE_MINUTE_MS = 60 * 1000;
const T_PLUS_30H_HOURS = 30;
const T_PLUS_30H_MS = T_PLUS_30H_HOURS * 60 * 60 * 1000;
const DEFAULT_WINDOW_MIN = 15;

// 디폴트 LIVE +30h 시점 (5/8 07:00 KST submit 추정 + 30h). 본 값은 추정 — submitKst 인자 우선.
export const LIVE_PLUS_30H_KST = "2026-05-09T13:00:00+09:00";
...
function envWindowMin() {
  const v = process.env.LIVE_PLUS_30H_WINDOW_MIN;
...
export default function checkLivePlus30h({
  submitKst,
  nowKst,
  windowMin,
  freeze = false,
  ...
  const targetMs = submitMs + T_PLUS_30H_MS;
...
function main() {
  ...
  try {
    const result = checkLivePlus30h({
      submitKst: args.submitKst,
      nowKst: args.nowKst,
      windowMin: args.windowMin === null ? undefined : parseInt(args.windowMin, 10),
      freeze: args.freeze,
    });
    console.log(JSON.stringify({ livePlus30h: result }));
    console.log(
      `check:live-plus-30h ok=${result.ok} deltaMin=${result.deltaMin}${result.reason ? ` reason=${result.reason}` : ""}`,
    );
    process.exit(result.ok ? 0 : 1);
  } catch (e) {
    console.error(`check:live-plus-30h ERROR: ${e.message}`);
    process.exit(1);
  }
}
