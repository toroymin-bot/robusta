#!/usr/bin/env node
/**
 * sim-live-plus-192h.mjs
 *   - C-D73-4 (D+2 23시 §6 슬롯, 2026-05-10) — Tori spec C-D73-4 (§8.5 / C-D73-1 보조).
 *
 * Why: C-D73-1 check-live-plus-192h.mjs 6 케이스 dry-run 시뮬레이터 (D-72 패턴 미러).
 *   각 케이스 expected 값 hardcoded + 실 실행 결과 매칭 + 6/6 PASS 의무.
 *
 * 6 케이스 매트릭스 (§8.5):
 *   C1: T+192h 정확 (windowMin=60)              → ok=true,  inWindow=true
 *   C2: T+191h 정각 (windowMin=60, lower)       → ok=true
 *   C3: T+193h 정각 (windowMin=60, upper)       → ok=true
 *   C4: T+190h59m (windowMin=60)                → ok=false (out of window)
 *   C5: T+193h01m (windowMin=60)                → ok=false (out of window)
...
import { checkLivePlus192h } from "./check-live-plus-192h.mjs";

const LIVE_START_ISO = "2026-05-08T00:00:00+09:00";
const ONE_HOUR_MS = 60 * 60 * 1000;
const ONE_MINUTE_MS = 60 * 1000;
const NON_EXISTENT_PINGS = "data/analytics/__nonexistent_d73.jsonl";
...
export function simLivePlus192h() {
  const start = Date.now();
  const cases = [];

  // C1: T+192h 정각 (windowMin=60).
  {
    const r = safeRun(() => checkLivePlus192h({
    ...
    cases.push({
      name: "C1: T+192h 정각 → ok=true inWindow=true",
  ...
  // C4: T+190h59m → ok=false.
  {
    ...
    cases.push({
      name: "C4: T+190h59m → ok=false (out of window)",
      expected: { ok: false },
  ...
  // C5: T+193h01m → ok=false.
  {
    ...
    cases.push({
      name: "C5: T+193h01m → ok=false (out of window)",
      expected: { ok: false },
      actual: r.ok ? { ok: r.result.ok, reasons: r.result.reasons } : { error: r.error },
      match: r.ok && r.result.ok === false && r.result.reasons.includes("out of window"),
    });
...
function main() {
  ...
  console.log("");
  console.log(
    `sim:live-plus-192h — ${result.pass}/${result.total} ${result.fail === 0 ? "PASS" : "FAIL"} (${result.totalMs} ms)`,
  );
