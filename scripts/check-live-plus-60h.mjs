#!/usr/bin/env node
/**
 * check-live-plus-60h.mjs
 *   - C-D67-1 (D+1 15시 §8 슬롯, 2026-05-09) — Tori spec C-D67-1 (§20.5.1).
 *
 * Why: LIVE +60h ±15분 윈도우 정형 lock (C-D66-1 mirror).
 *   LIVE 시작 후 60h 시점 ±windowMin 정합 검증 read-only 게이트.
 *   외부 fetch 0건 / Dexie 미접근 / 본 도구는 시점 산식만 책임.
 *
 * 자율 정정 큐 (D-67-자-1):
 *   환경 변수 LIVE_PLUS_60H_WINDOW_MIN 으로 windowMin override 가능 (디폴트 15 보존, backward-compatible).
 *
 * 함수 시그니처 (named export, §20.5.1 본체 lock 정합):
 *   checkLivePlus60h(now: Date, opts?: { startedAtIso?: string, windowMin?: number })
 ...
 *     - opts.windowMin:     윈도우 ±분, 미지정 시 process.env.LIVE_PLUS_60H_WINDOW_MIN 폴백 → 미존재 시 기본 15.
 *
 * 출력 정의:
 *   withinWindow  60h 앵커 ±windowMin 범위 내 boolean.
 *   ageH          (now - startedAt) 시간 (소수점 2자리, 음수 가능).
 *   deltaMin      60h 앵커 대비 분 차이 (±부호, 정수).
 *   reason        within / before / after — 케이스 라벨.
 *
 * 엣지 케이스:
 *   1) opts.startedAtIso 미지정 + env 미존재 → throw new Error('LIVE start unknown')
 *   2) windowMin = 0 → 정확히 60.000h 만 within (deltaMin === 0 ↔ withinWindow=true)
 ...
 *
 * 사용 (CLI):
 *   $ node scripts/check-live-plus-60h.mjs --started=2026-05-08T07:00:00+09:00
 *   $ LIVE_STARTED_AT=2026-05-08T07:00:00+09:00 node scripts/check-live-plus-60h.mjs
 *
...
const T_PLUS_60H_HOURS = 60;
const T_PLUS_60H_MS = T_PLUS_60H_HOURS * ONE_HOUR_MS;
...
function envWindowMin() {
  const v = process.env.LIVE_PLUS_60H_WINDOW_MIN;
...
export function checkLivePlus60h(now, opts) {
  const o = opts || {};
  ...
  const targetMs = startedMs + T_PLUS_60H_MS;
...
function main() {
  ...
  try {
    ...
    const result = checkLivePlus60h(now, {
      startedAtIso: args.startedAtIso ?? undefined,
      windowMin: args.windowMin === null ? undefined : parseInt(args.windowMin, 10),
    });
    console.log(JSON.stringify({ livePlus60h: result }));
    console.log(
      `check:live-plus-60h withinWindow=${result.withinWindow} ageH=${result.ageH} deltaMin=${result.deltaMin} reason=${result.reason}`,
    );
    process.exit(result.withinWindow ? 0 : 1);
  } catch (e) {
    console.error(`check:live-plus-60h ERROR: ${e.message}`);
    process.exit(1);
  }
}
