 ...
 *     · 누적 0건 시 "⚡ 키 등록 시 누적 표시" placeholder 박음 → 클릭 시 onRequestApiKeyModal 호출.
 *     · 다크 모드: bg-robusta-accent/25 text-robusta-paper, 라이트: bg-robusta-accent/15 text-robusta-ink.
 *     · prop onRequestApiKeyModal 미박힘 시 placeholder 클릭 noop (defensive default).
 *   - C-D17-22 (Day 5 19시 슬롯, 2026-04-30) — D-19 tabular-nums 클래스 박음 (숫자 폭 균일).
...
interface TokenCounterBadgeProps {
  /** placeholder 클릭 시 호출. 누적 ≥ 1건이면 호출 X. 미박힘 시 noop (defensive). */
...
export function TokenCounterBadge({
  ...
  // C-D17-19 F-17: 0건 시 placeholder 박음 (이전엔 null 반환).
